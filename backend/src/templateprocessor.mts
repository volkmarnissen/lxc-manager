...
import { IReadApplicationOptions, ProxmoxLoadApplicationError } from "@src/proxmoxconfiguration.mjs";
import { IConfiguredPathes, ProxmoxConfigurationError } from "@src/proxmoxconftypes.mjs";
...
export class TemplateProcessor {
 ...
 loadApplication(applicationName: string, task: TaskType): ITemplateProcessorLoadResult {
    ...
    const scriptPathes = readOpts.applicationHierarchy.map((appDir) =>
     ...
     templatePathes.push(
       path.join(this.pathes.jsonPath, "shared", "templates"),
     );
     scriptPathes.push(
       path.join(this.pathes.jsonPath, "shared", "scripts"),
     ); 
     ...
     return {parameters: outParameters, commands: outCommands, resolvedParams: resolvedParams};
   }
   ...
   #processTemplate(opts: ProxmoxProcessTemplateOpts): void {
     ...
     if (!tmplPath) {
       opts.errors.push(
         new Error(
           `Template file not found: ${opts.template} (searched in: ${opts.templatePathes.join(", ")}` + ', requested in: ${opts.requestedIn ?? "unknown"}${opts.parentTemplate ? ", parent template: " + opts.parentTemplate : ""})',
         ),
       );
       return;
     }
    ...
    getUnresolvedParameters(parameters:IParameter[],resolvedParams: Set<string>): IParameter[] {
    return parameters.filter(
      (param) => !resolvedParams.has(param.name),
    );
  }
}