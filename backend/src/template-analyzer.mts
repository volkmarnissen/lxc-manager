...
export class TemplateAnalyzer {
  ...
  constructor(pathResolver: DocumentationPathResolver, configuredPathes: IConfiguredPathes) {
        ...
        try {
          ...
          if (appData.installation) {
            ...
            for (const templateRef of appData.installation) {
              ...
              const normalizedRef = this.pathResolver.normalizeTemplateName(refTemplateName);
              ...
              for (const templateRef of appData.installation) {
                ...
                if (templateData) {
                  ...
                  for (const refTemplateName of referencedTemplates) {
                    const cmdTemplateName = this.pathResolver.normalizeTemplateName(refTemplateName);
